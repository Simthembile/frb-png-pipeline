...
for ax, (alpha, label) in zip(axes, [
    ...
    ax.loglog(ELL_ARR, fac * cDD / norm_DD,
              ...
              label=r'$C^{\rm DD}(\ell)$')
    cDT_plot = np.abs(cDT)
    cDT_plot = np.where(cDT_plot > 0, cDT_plot, np.nan)   # mask zeros for loglog
    ax.loglog(ELL_ARR, fac * cDT_plot / norm_DD,
              color=RED,    lw=2.0, ls='--',
              label=r'$|C^{{\rm D}\Delta t}(\ell)|$')
    # C^tt rescaled to 80% of C^DD peak for visual comparison
    ax.loglog(ELL_ARR, fac * cTT / norm_tt * 0.8,
              color=ORANGE, lw=2.0, ls=':',
              label=r'$C^{\Delta t\Delta t}(\ell)$ [rescaled]')

    # Noise levels (scaled same way)
    ax.axhline(NDD / norm_DD,           color=BLUE,   ls=(0,(4,2)), lw=1.0, alpha=0.55)
    ax.axhline(Ntt / norm_tt * 0.8,     color=ORANGE, ls=(0,(4,2)), lw=1.0, alpha=0.55)
